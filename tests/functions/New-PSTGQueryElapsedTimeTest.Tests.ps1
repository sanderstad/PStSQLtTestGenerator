...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        ...
        [object[]]$knownParameters = 'SqlInstance', 'SqlCredential', 'Database', 'Schema', 'Function', 'OutputPath',
            'Creator', 'TemplateFolder', 'TestClass', 'Query', 'UseLastElapsedTime', 'UseExecute', 'InputObject', 'EnableException'
...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Create Query Elapsed Test" {
        $result = @()
        $result += New-PSTGQueryElapsedTimeTest -SqlInstance $script:sqlinstance -Database $script:database -OutputPath $script:unittestfolder -EnableException
