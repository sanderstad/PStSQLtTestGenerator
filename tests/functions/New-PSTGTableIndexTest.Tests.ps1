...
Describe "$commandname Integration Tests" -Tags "IntegrationTests" {
    ...
    Context "Create Table Column Test" {
        $result = @()
        $result += New-PSTGTableIndexTest -SqlInstance $script:sqlinstance -Database $script:database -OutputPath $script:unittestfolder -EnableException
    ...
    Context "Using Pipeline" {
        ...
        $result = @()
        $result += $tables.Name | New-PSTGTableIndexTest -SqlInstance $script:sqlinstance -Database $script:database -OutputPath $script:unittestfolder -EnableException
