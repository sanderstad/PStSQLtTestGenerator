...
$parentModule = '_PSFProject'
if (-not $ModuleName) { $ModuleName = '_PSFProject.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\_PSFProject'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\_PSFProject.psd1"
$prereqHash = @{
...
if ($LocalRepo)
{
	...
	Write-PSFMessage -Level Important -Message "Creating Nuget Package for module: _PSFProject"
	New-PSMDModuleNugetPackage -ModulePath $workingRoot.FullName -PackagePath . -EnableException
}
else
{
	# Publish to Gallery
	Write-PSFMessage -Level Important -Message "Publishing the _PSFProject module to $($Repository)"
